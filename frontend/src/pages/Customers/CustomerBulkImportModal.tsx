...
export const CustomerBulkImportModal: React.FC<CustomerBulkImportModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
}) => {
  ...
  const handleDownloadTemplate = async () => {
    ...
    try {
      const blob = await contactService.downloadCustomerTemplate();
      const url = window.URL.createObjectURL(new Blob([blob]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', 'Customer_Bulk_Import_Template.xlsx');
    ...
    reader.onload = (evt) => {
      try {
        ...
        headers.forEach((h, idx) => {
          if (h.includes('name') || h.includes('customer') || h.includes('client')) map.name = idx;
          else if (h.includes('code') || h.includes('id')) map.customer_id = idx;
          else if (h.includes('phone') || h.includes('mobile') || h.includes('cell') || h.includes('contact')) map.phone = idx;
          else if (h.includes('email') || h.includes('mail')) map.email = idx;
          else if (h.includes('address') || h.includes('location') || h.includes('city')) map.address = idx;
          else if (h.includes('credit') || h.includes('allow') || h.includes('eligib')) map.credit_enabled = idx;
          else if (h.includes('open') || h.includes('balance') || h.includes('receiv')) map.opening_balance = idx;
          else if (h.includes('note') || h.includes('remark') || h.includes('desc')) map.notes = idx;
        });
        ...
        for (let i = 1; i < json.length; i++) {
          ...
          const rowObj: any = {
            ...
            customer_id: map.customer_id !== undefined ? String(rowData[map.customer_id] || '').trim() : '',
            ...
            credit_enabled: map.credit_enabled !== undefined ? String(rowData[map.credit_enabled] || 'Yes').trim() : 'Yes',
        ...
        if (rows.length === 0) {
          setError('No valid customer rows found in the sheet. Please ensure column headers match the template.');
        }
  ...
  const handleStartImport = async () => {
    if (parsedRows.length === 0) {
      setError('Please select a valid file containing customer records.');
      return;
    }
    ...
    try {
      const res: ContactBulkImportResult = await contactService.bulkImportCustomers(parsedRows);
      setResult(res);
      if (res.created_count > 0) {
        onSuccess();
      }
    } catch (err: any) {
      setError(err?.response?.data?.detail || err?.message || 'Customer bulk import failed.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
            ...
            <div style={{ fontSize: '0.8125rem' }}>
              <strong style={{ color: 'var(--text-main)' }}>Need the formatted customer template?</strong>
              <div style={{ color: 'var(--text-muted)' }}>
                Download our sample Excel spreadsheet with pre-configured headers and example customer rows.
              </div>
            </div>
          </div>
          <Button
        ...
        {result ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div style={{
              ...
              <p style={{ fontSize: '0.875rem', color: 'var(--text-muted)' }}>
                Successfully created <strong style={{ color: 'var(--success)' }}>{result.created_count}</strong> customers
          ...
          <>
            ...
            <div
              ...
              onClick={() => document.getElementById('customer-bulk-import-file-input')?.click()}
            >
              <input
                id="customer-bulk-import-file-input"
                type="file"
                accept=".xlsx,.xls,.csv"
                style={{ display: 'none' }}
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.75rem', marginTop: '0.5rem' }}>
              ...
              >
                Import {parsedRows.length > 0 ? `${parsedRows.length} Customers` : 'Customers'}
              </Button>
            </div>
          </>
        )}
