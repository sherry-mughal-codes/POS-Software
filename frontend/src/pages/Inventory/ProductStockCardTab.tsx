      ...
      {loading ? (
        ...
        <>
          ...
          <div className="glass-card" style={{ padding: '1.5rem' }}>
            ...
            {/* Metrics Breakdown */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: '1rem', paddingTop: '1rem', borderTop: '1px solid var(--border-subtle)' }}>
              <div>
                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: 600 }}>Current On-Hand Stock</span>
                <div style={{ fontSize: '1.5rem', fontWeight: 800, fontFamily: 'var(--font-mono)', color: 'var(--text-main)', marginTop: '0.25rem' }}>
                  {stockCard.current_stock} <span style={{ fontSize: '0.875rem', color: 'var(--text-muted)' }}>{stockCard.unit}</span>
                </div>
              </div>

              <div>
                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: 600 }}>Weighted Average Cost (WAC)</span>
                <div style={{ fontSize: '1.5rem', fontWeight: 800, fontFamily: 'var(--font-mono)', color: 'var(--text-main)', marginTop: '0.25rem' }}>
              ...
              <div>
                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: 600 }}>Total Inventory Valuation</span>
                <div style={{ fontSize: '1.5rem', fontWeight: 800, fontFamily: 'var(--font-mono)', color: 'var(--primary-400)', marginTop: '0.25rem' }}>
                  Rs. {formatMoney(stockCard.total_valuation)}
                </div>
              </div>

              <div>
                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: 600 }}>Lifetime Inward (+Stock)</span>
                <div style={{ fontSize: '1.5rem', fontWeight: 800, fontFamily: 'var(--font-mono)', color: 'var(--success)', marginTop: '0.25rem' }}>
                  +{stockCard.total_stock_in} <span style={{ fontSize: '0.875rem', color: 'var(--text-muted)' }}>{stockCard.unit}</span>
                </div>
              </div>

              <div>
                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: 600 }}>Lifetime Outward (-Stock)</span>
                <div style={{ fontSize: '1.5rem', fontWeight: 800, fontFamily: 'var(--font-mono)', color: 'var(--danger)', marginTop: '0.25rem' }}>
                  -{stockCard.total_stock_out} <span style={{ fontSize: '0.875rem', color: 'var(--text-muted)' }}>{stockCard.unit}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <Card
            title={`Stock Card Transaction History (${stockCard.timeline.length} movements)`}
            subtitle="Complete mathematical breakdown tracing every movement from inception to current on-hand balance"
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.875rem' }}>
                <thead>
                  <tr style={{ borderBottom: '1px solid var(--border-medium)', color: 'var(--text-muted)' }}>
                    ...
                    <th style={{ padding: '0.75rem 1rem', fontWeight: 600, textAlign: 'right' }}>Inward (+)</th>
                    <th style={{ padding: '0.75rem 1rem', fontWeight: 600, textAlign: 'right' }}>Outward (-)</th>
                    <th style={{ padding: '0.75rem 1rem', fontWeight: 600, textAlign: 'right' }}>Unit Cost Rate</th>
                    <th style={{ padding: '0.75rem 1rem', fontWeight: 600, textAlign: 'right' }}>Running Balance</th>
                    <th style={{ padding: '0.75rem 1rem', fontWeight: 600 }}>Reference Doc</th>
                  ...
                  {stockCard.timeline.length === 0 ? (
                    ...
                    stockCard.timeline.map((row) => (
                      ...
                      >
                        ...
                        <td style={{ padding: '0.875rem 1rem', textAlign: 'right', fontFamily: 'var(--font-mono)', fontWeight: 700, color: 'var(--danger)' }}>
                          {row.quantity < 0 ? `${row.quantity}` : '-'}
                        </td>
                        ...
                        <td style={{ padding: '0.875rem 1rem' }}>
                          {row.reference_id ? (
                            <div>
                              <code style={{ fontSize: '0.8125rem', fontWeight: 600 }}>{row.reference_id}</code>
                              {row.notes && (
